...
# Note: pmic_enable functionality now handled by rk_pmic_enable below
...
# RK PMIC enable pin - handles both main power control and emergency signaling
# (replaces both old pmic_enable and cm5_power_btn functionality)
rk_pmic_enable = machine.Pin(3, machine.Pin.OUT, value=0)  # CM5 OFF by default
...
if upBTN.value() and not selectBTN.value() and not downBTN.value():
    ...
    rk_pmic_enable.value(1)  # Power on CM5 for firmware upload
    switch_usb("SOM_USB")
...
def set_cm5_power(enable):
    """Control CM5 power via rk_pmic_enable pin (regular output mode)
    
    Args:
        enable (bool): True to power on (high), False to power off (low)
    """
    global power_state
    
    try:
        if enable:
            # Power on: Set pin high - CM5 can power on
            rk_pmic_enable.value(1)
            power_state["cm5_powered"] = True
            debug.log_info(debug.CAT_POWER, "CM5 powered ON (rk_pmic_enable -> high)")
            set_debug_color("UART_READY")
        else:
            # Power off: Set pin low - CM5 powers off
            rk_pmic_enable.value(0)
            power_state["cm5_powered"] = False
            debug.log_info(debug.CAT_POWER, "CM5 powered OFF (rk_pmic_enable -> low)")
...
def check_emergency_battery_shutdown():
    ...
    try:
        ...
        if not is_charging and battery_percent == 1:
            ...
            # Emergency shutdown: directly power off CM5 via rk_pmic_enable
            # Since we now use the same pin for power control and signaling,
            # emergency shutdown directly cuts power instead of signaling
            debug.log_info(debug.CAT_POWER, "Emergency shutdown: powering off CM5 immediately")
            
            # Clear any pending shutdown states before emergency power off
            power_state["shutdown_pending"] = False
            power_state["shutdown_timer"] = 0
            
            # Immediately power off CM5
            set_cm5_power(False)
...
def eink_boot_screen_task():
    ...
    try:
        ...
        # Use 2.1 inch eink initialization logic
        einkRunning = True
        if not eink.init:
            eink.re_init()
        ...
        wdt.feed()
        # Animation loop with additional power state checks - continue until display release signal received or max cycles reached
        cycle = 0
        while einkRunning and not display_release_received and power_state["cm5_powered"] and cycle < EINK_ANIMATION_TIMEOUT:
            with eink_lock:
                if not einkRunning or cycle >= EINK_ANIMATION_TIMEOUT:
                    break
            ...
            try:
                ...
                debug.log_verbose(debug.CAT_DISPLAY, f"Animation cycle {cycle} completed")
        ...
        if display_release_received:
            debug.log_info(debug.CAT_DISPLAY, f"Animation ended after {cycle} cycles - CM5 boot signal received")
        elif not power_state["cm5_powered"]:
            debug.log_info(debug.CAT_DISPLAY, f"Animation ended after {cycle} cycles - CM5 powered off")
        elif cycle >= EINK_ANIMATION_TIMEOUT:
            debug.log_info(debug.CAT_DISPLAY, f"Animation ended after {cycle} cycles - timeout reached ({EINK_ANIMATION_TIMEOUT} max)")
        else:
            debug.log_info(debug.CAT_DISPLAY, f"Animation ended after {cycle} cycles - completed")
...
def eink_display_task(image_paths):
    ...
    try:
        ...
        if len(image_paths) == 1:
            ...
            image_index = 0
            cycle = 0
            while einkRunning and not power_state["cm5_powered"] and cycle < EINK_ANIMATION_TIMEOUT:
                wdt.feed()
                with eink_lock:
                    if not einkRunning or cycle >= EINK_ANIMATION_TIMEOUT:
                        break
                ...
                try:
                    ...
                    with open(image_paths[next_index], "rb") as f:
                        next_image_data = f.read()
                    eink.epd_display_part_all(next_image_data)
                    ...
                    debug.log_verbose(debug.CAT_DISPLAY, f"Animation cycle {cycle} completed (image {current_index} -> {next_index})")
        ...
        elif len(image_paths) > 1 and cycle >= EINK_ANIMATION_TIMEOUT:
